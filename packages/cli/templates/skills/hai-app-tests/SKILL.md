...
| 项目 | 契约 |
| --- | --- |
| 能力 | 以 TDD 驱动应用开发：先分析需求、生成单元测试（Vitest）与 E2E 测试（Playwright），确认测试全部失败后再编码实现直至测试通过；当需求涉及应用测试、TDD 流程、覆盖率提升、E2E 测试时使用。 |
| 适用场景 | 当任务与 `hai-app-tests` 的能力描述匹配，并且需要遵循本 Skill 的流程和边界时 |
