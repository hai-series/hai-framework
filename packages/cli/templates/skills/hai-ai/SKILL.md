---
name: hai-ai
description: 使用 @h-ai/ai 进行 LLM 调用（OpenAI 兼容）、MCP 服务器创建、工具定义与注册、流式处理、记忆管理、上下文压缩；当需求涉及 AI 对话、工具调用、MCP 协议、流式响应、记忆提取注入、上下文摘要压缩或 AI 客户端时使用。
---

# hai-ai

> `@h-ai/ai` 提供统一的 AI 能力，支持 LLM 调用（OpenAI 兼容 API）、MCP 服务器、工具定义与注册、流式处理、记忆管理、上下文压缩与摘要。支持 Node.js 与浏览器双端。
...
| `inject`  | `(messages, options?) => Promise<Result<ChatMessage[]>>`  | 将相关记忆自动注入消息列表       |
| `remove`  | `(memoryId) => Promise<Result<void>>`                     | 删除指定记忆                       |
| `list`    | `(options?) => Promise<Result<MemoryEntry[]>>`            | 列出记忆（支持类型/来源过滤）       |
| `clear`   | `(options?) => Promise<Result<void>>`                     | 清空记忆（支持按类型/来源清空） |
...
const enriched = await ai.memory.inject(messages, { topK: 5, position: 'system' })
if (enriched.success) {
...
| 方法             | 签名                                                          | 说明                                      |
| ---------------- | ------------------------------------------------------------- | ----------------------------------------- |
| `compress`       | `(messages, options?) => Promise<Result<ContextCompressResult>>` | 压缩消息列表至指定 Token 预算         |
...
async function chat(userInput: string) {
  ...
  // 1. 注入相关记忆到消息列表
  const enriched = await ai.memory.inject(messages, {
    topK: 5,
    position: 'system',
  })
  ...
  // 2. 压缩上下文以适应 Token 限制
  const compressed = await ai.context.compress(enriched.data, {
    maxTokens: 4000,
    strategy: 'hybrid',
  })
...
async function chat(userInput: string) {
  ...
  await manager.append(result.data.choices[0].message)
