...
// 自动加载 config/_core.yml 及同目录下的所有 _*.yml
await core.init()
...
| 方法         | 签名                                                             | 说明                         |
| ------------ | ---------------------------------------------------------------- | ---------------------------- |
| `core.init`  | `(config?: CoreConfigInput) => Promise<Result<void, CoreError>>` | 初始化（加载配置/日志/i18n） |
| `core.close` | `() => Promise<Result<void, CoreError>>`                         | 关闭所有资源                 |
...
| `getAll`   | `() => Record<string, unknown>`                           | 获取完整配置           |
| `validate` | `(name: string, schema: ZodType) => Result<T, CoreError>` | 用 Zod Schema 校验配置 |
| `set`      | `(name: string, config: unknown) => void`                 | 运行时更新配置         |
...
```typescript
const logger = core.logger.createModuleLogger('my-module')
logger.info('Module initialized')
// 输出: [my-module] Module initialized
```
...
| 方法                      | 签名                           | 说明                                     |
| ------------------------- | ------------------------------ | ---------------------------------------- |
| `createNotInitializedKit` | `<T>(moduleName: string) => T` | 创建未初始化代理（访问任何方法返回错误） |

```typescript
// 在模块 main.ts 中使用
let currentService: MyService = core.module.createNotInitializedKit('my-module')

export const myModule = {
  async init(config) {
    currentService = createService(config)
    return ok(undefined)
  },
  get service() {
...
| `ALREADY_INITIALIZED`     | 模块已初始化                 |
| `CONFIG_ERROR`            | 配置错误（缺失或格式不合法） |
| `CONFIG_VALIDATION_ERROR` | 配置校验失败（Zod）          |
