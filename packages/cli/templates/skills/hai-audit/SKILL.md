---
name: hai-audit
description: "使用 @h-ai/audit 记录、查询、统计和清理操作审计。"
---
...
| 项目 | 契约 |
| --- | --- |
| 能力 | 使用 @h-ai/audit 记录、查询、统计和清理操作审计 |
| 适用场景 | 业务操作留痕与审计记录检索 |
| 输入 | AuditConfigInput、已初始化 reldb、审计事件与查询条件 |
| 输出 | 记录/查询/统计的 HaiResult |
| 限制 | 仅服务端；审计内容需脱敏，保留期和清理范围由应用策略决定。 |
