...
describe('db.crud edge cases', () => {
  const defineCommon = (label: 'sqlite' | 'mysql' | 'postgresql') => {
    ...
    it(`${label}: create 空对象应返回 CONFIG_ERROR`, async () => {
      await ensureTable()
      const crud = db.crud.table<UserRow>({
        table: 'users',
        idColumn: 'id',
    ...
    it(`${label}: mapRow 自定义映射应在 findAll/findById 生效`, async () => {
      ...
      const crud = db.crud.table<MappedUser>({
        table: 'users',
        idColumn: 'id',
        select: ['id', 'name'],
        createColumns: ['name', 'email'],
    ...
    it(`${label}: 不指定 createColumns 时应接受所有字段`, async () => {
      ...
      const check = await db.sql.get<{ status: string }>('SELECT status FROM users WHERE id = 1')
