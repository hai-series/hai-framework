/**
 * =============================================================================
 * @h-ai/db - BaseCrudRepository 类型转换测试
 * =============================================================================
 *
 * 覆盖 BaseCrudRepository 的值类型转换场景：
...
const ARTICLE_FIELDS: CrudFieldDefinition[] = [
  {
    fieldName: 'id',
    columnName: 'id',
...
class ArticleRepository extends BaseCrudRepository<ArticleRow> {
  constructor() {
    super(db, {
      table: 'articles',
      idColumn: 'id',
...
const NOTE_FIELDS: CrudFieldDefinition[] = [
  {
    fieldName: 'id',
    columnName: 'id',
...
class NoteRepository extends BaseCrudRepository<NoteRow> {
  private counter = 0
...
describe('db.BaseCrudRepository types', () => {
  const defineCommon = (label: 'sqlite' | 'mysql' | 'postgresql') => {
    ...
    it(`${label}: 自定义 nowProvider 应使用固定时间`, async () => {
      await db.ddl.dropTable('notes', true)
    ...
    // ─── 事务中使用 BaseCrudRepository ───

    it(`${label}: BaseCrudRepository 在事务中 create 后 rollback 应撤回`, async () => {
      await db.ddl.dropTable('articles', true)
      const repo = new ArticleRepository()
    ...
    it(`${label}: createTableIfNotExists=false 时不应自动建表`, async () => {
      await db.ddl.dropTable('manual_table', true)

      class ManualRepo extends BaseCrudRepository<{ id: number, val: string }> {
        constructor() {
          super(db, {
            table: 'manual_table',
            idColumn: 'id',
