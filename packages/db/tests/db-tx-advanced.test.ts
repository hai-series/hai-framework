...
describe('db.tx advanced', () => {
  const defineCommon = (label: 'sqlite' | 'mysql' | 'postgresql') => {
    const ensureTables = async () => {
      await db.ddl.dropTable('orders', true)
      ...
      await db.ddl.createTable('orders', {
        id: { type: 'INTEGER', primaryKey: true, autoIncrement: true },
    ...
    it(`${label}: begin+commit 后再 execute 应返回 TRANSACTION_FAILED`, async () => {
      ...
      if (!afterCommit.success) {
        expect(afterCommit.error.code).toBe(DbErrorCode.TRANSACTION_FAILED)
      }
    })
    ...
    it(`${label}: begin+rollback 后再 execute 应返回 TRANSACTION_FAILED`, async () => {
      ...
      if (!afterRollback.success) {
        expect(afterRollback.error.code).toBe(DbErrorCode.TRANSACTION_FAILED)
      }
    })
    ...
    it(`${label}: commit 后再次 commit 应返回 TRANSACTION_FAILED`, async () => {
      ...
      if (!second.success) {
        expect(second.error.code).toBe(DbErrorCode.TRANSACTION_FAILED)
      }
    })
    ...
    it(`${label}: 跨表事务提交应同时生效`, async () => {
      ...
      const user = await db.sql.get<{ balance: number }>('SELECT balance FROM users WHERE id = 1')
      expect(user.success).toBe(true)
      if (user.success) {
        expect(user.data?.balance).toBe(800)
      }

      const order = await db.sql.get<{ amount: number }>('SELECT amount FROM orders WHERE user_id = 1')
    ...
    it(`${label}: 跨表事务回滚应全部撤回`, async () => {
      ...
      // 先插入一个用户
      await db.sql.execute('INSERT INTO users (name, balance) VALUES (?, ?)', ['买家', 1000])
      ...
      // 订单不应存在
      const orders = await db.sql.query('SELECT * FROM orders')
    ...
    it(`${label}: begin 模式下的 tx.crud 操作 + 手动 rollback 应撤回`, async () => {
      ...
      // 回滚后应无数据
      const afterCount = await db.sql.get<{ cnt: number }>('SELECT COUNT(*) as cnt FROM orders')
    ...
    it(`${label}: 连续多次 tx.wrap 应独立完成`, async () => {
      ...
      // 第一个事务
      const r1 = await db.tx.wrap(async (tx) => {
        await tx.execute('INSERT INTO users (name, balance) VALUES (?, ?)', ['用户1', 100])
        return 1
      })
      ...
      // 第二个事务
      const r2 = await db.tx.wrap(async (tx) => {
      ...
      // 第三个事务（回滚）
      const r3 = await db.tx.wrap(async (tx) => {
      ...
      // 应只有前两条
      const count = await db.sql.get<{ cnt: number }>('SELECT COUNT(*) as cnt FROM users')
    ...
    it(`${label}: tx.wrap 内 batch 失败应整体回滚`, async () => {
      ...
      await db.sql.execute('INSERT INTO users (name, balance) VALUES (?, ?)', ['原始用户', 100])
    ...
    it(`${label}: begin 模式下 queryPage 应读取事务快照`, async () => {
      ...
      // 回滚后应无数据
      const after = await db.sql.query('SELECT * FROM users')
