...
import type { LogFormat, LogLevel, LoggingConfig } from "./config/core-config-app";
...
/**
 * 统一错误码类型
 * 错误码定义分布在各个 Schema 模块中：
 * - CommonErrorCode (1000-1099): 通用错误 - schemas/app.ts
 * - ConfigErrorCode (1100-1199): 配置错误 - schemas/app.ts
 * - AuthErrorCode (2000-2999): 认证错误 - schemas/iam.ts
 * - DbErrorCode (3000-3999): 数据库错误 - schemas/db.ts
 * - AIErrorCode (4000-4999): AI 错误 - schemas/ai.ts
 * - StorageErrorCode (5000-5999): 存储错误 - schemas/storage.ts
 * - CryptoErrorCode (6000-6999): 加密错误 - schemas/crypto.ts
 */
...
/** Provider 配置 Map - 用于存储各 Provider 的独立配置 */
export type ProviderConfigMap = Map<string, Record<string, unknown>>
