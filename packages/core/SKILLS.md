...
// 校验已加载的配置（模块使用 configDir 加载的配置前，需显式校验）
...
// 创建异步占位 Proxy（默认，所有方法返回 Promise<Result>）
const notInitializedOps = notInitialized.proxy<XxOperations>()

// 创建同步占位 Proxy（所有方法返回 Result）
const syncOps = notInitialized.proxy<XxSyncOperations>('sync')
