 ...
 * =============================================================================
 *
 * 本文件定义部署模块的核心接口和类型（非配置相关）。
 * 配置相关类型请从 deploy-config.ts 导入。
 *
 * 包含：
 * - 错误类型（DeployError）
 * - 扫描结果（ScanResult）
 * - 部署结果（DeployResult）
 * - 开通结果（ProvisionResult）
 * - 部署平台 Provider 接口（DeployProvider）
 * - 基础设施 Provisioner 接口（ServiceProvisioner）
 * - 模块功能接口（DeployFunctions）
 *
 * @example
 * ```ts
 * import type { DeployFunctions, DeployProvider, ScanResult } from '@h-ai/deploy'
 * ```
 *
 * @module deploy-types
 * =============================================================================
