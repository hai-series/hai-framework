...
> 此文件描述 @h-ai/kit 模块的架构、API 与集成方式，供 AI 助手参考。

## 1. 模块概述

`@h-ai/kit` 是 hai-framework 与 SvelteKit 的集成层，将框架各模块（IAM / Cache / Storage / Crypto）以 SvelteKit 惯用方式暴露为 Handle Hook、中间件、路由守卫、API 响应工具、表单验证与客户端 Store。
