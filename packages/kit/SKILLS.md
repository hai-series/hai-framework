...
packages/kit/
  ...
  messages/
    ...
    zh-CN.json                            # 中文消息
  src/
    index.ts                              # 主入口（export * 聚合 + i18n 初始化）
    types.ts                              # 公共类型（SessionData, HookConfig, Middleware 等）
    response.ts                           # API 响应工具（ok, created, error 等）
    validation.ts                         # 表单验证（validateForm/Query/Params）
    guards/
...
```typescript
// 主入口 — 包含全部服务端 + 客户端导出
import { authGuard, createHandle, ok, useSession, validateForm } from '@hai/kit'

// 客户端 Store
import { useIsAuthenticated, useSession, useUpload, useUser } from '@hai/kit/client'
import { createCacheHandle, createCacheUtils } from '@hai/kit/modules/cache'
...
| ------------------- | ------------------------------- |
| `.`                 | `dist/index.js`                 |
| `./modules/iam`     | `dist/modules/iam/index.js`     |
| `./modules/storage` | `dist/modules/storage/index.js` |
| `./modules/cache`   | `dist/modules/cache/index.js`   |
| `./modules/crypto`  | `dist/modules/crypto/index.js`  |
...
### createHandle(config?: HookConfig): Handle
...
### sequence(...handles: Handle[]): Handle

组合多个 SvelteKit handle，从左到右嵌套执行。

```typescript
export const handle = sequence(iamHandle, cacheHandle, appHandle)
```

## 6. 路由守卫

### authGuard(config?: AuthGuardConfig): RouteGuard
...
### roleGuard(config: RoleGuardConfig): RouteGuard
...
### permissionGuard(config: PermissionGuardConfig): RouteGuard
...
| 函数               | 签名                                                                   | 说明     |
| ------------------ | ---------------------------------------------------------------------- | -------- |
| `allGuards`        | `(...guards: RouteGuard[]) => RouteGuard`                              | AND 逻辑 |
| `anyGuard`         | `(...guards: RouteGuard[]) => RouteGuard`                              | OR 逻辑  |
| `notGuard`         | `(guard: RouteGuard, options?: { redirect?, message? }) => RouteGuard` | 取反     |
| `conditionalGuard` | `(condition: ConditionFn, guard: RouteGuard) => RouteGuard`            | 条件执行 |
...
```typescript
// 管理员 OR 拥有 posts:* 权限
anyGuard(roleGuard({ roles: ['admin'] }), permissionGuard({ permissions: ['posts:*'] }))
...
// 已登录用户不允许访问登录页
notGuard(authGuard(), { redirect: '/dashboard' })
```
...
### corsMiddleware(config?: CorsConfig): Middleware
...
### csrfMiddleware(config?: CsrfConfig): Middleware
...
### loggingMiddleware(config?: LoggingMiddlewareConfig): Middleware
...
### rateLimitMiddleware(config: RateLimitConfig): Middleware
...
| 函数              | 状态码 | 签名                                                                                                  |
| ----------------- | ------ | ----------------------------------------------------------------------------------------------------- |
| `ok`              | 200    | `<T>(data: T, requestId?: string) => Response`                                                        |
| `created`         | 201    | `<T>(data: T, requestId?: string) => Response`                                                        |
| `noContent`       | 204    | `() => Response`                                                                                      |
| `error`           | 自定义 | `(code: string, message: string, status?: number, requestId?: string, details?: unknown) => Response` |
| `badRequest`      | 400    | `(message: string, requestId?: string, details?: unknown) => Response`                                |
| `unauthorized`    | 401    | `(message?: string, requestId?: string) => Response`                                                  |
| `forbidden`       | 403    | `(message?: string, requestId?: string) => Response`                                                  |
| `notFound`        | 404    | `(message?: string, requestId?: string) => Response`                                                  |
| `conflict`        | 409    | `(message: string, requestId?: string) => Response`                                                   |
| `validationError` | 422    | `(errors: Array<{ field: string, message: string }>, requestId?: string) => Response`                 |
| `internalError`   | 500    | `(message?: string, requestId?: string) => Response`                                                  |
| `redirect`        | 3xx    | `(url: string, status?: 301\|302\|303\|307\|308) => Response`                                         |
...
### validateForm(request: Request, schema: ZodType): Promise\<FormValidationResult\>
...
### validateQuery(url: URL, schema: ZodType): FormValidationResult

从 `url.searchParams` 提取键值对并验证。

### validateParams(params: Record\<string, string\>, schema: ZodType): FormValidationResult
...
```typescript
import { ok, validateForm, validationError } from '@hai/kit'
import { z } from 'zod'

const Schema = z.object({ name: z.string().min(1), age: z.number().int().positive() })

export async function POST(event) {
  const { valid, data, errors } = await validateForm(event.request, Schema)
  if (!valid)
...
#### createIamHandle(config: IamHandleConfig): Handle
...
#### createIamActions(config: IamActionsConfig): Actions
...
#### requireAuth(iam: IamServiceLike): 端点级守卫

#### requireRole(iam: IamServiceLike, roles: string[]): 端点级角色守卫

#### requirePermission(iam: IamServiceLike, permission: string): 端点级权限守卫
...
#### createCacheHandle(config: CacheHandleConfig): Handle
...
#### createCacheUtils(config: { cache: CacheServiceLike })

返回缓存工具对象，包含缓存清除、预热等方法。

### Crypto 模块

#### verifyWebhookSignature(config: WebhookVerifyConfig): Promise\<boolean\>
...
#### signRequest(config): Promise\<string\>

生成请求签名。

#### createCsrfManager(config: CsrfConfig)
...
#### createEncryptedCookie(config: EncryptedCookieConfig)
...
#### createTransportEncryption(cryptoService: TransportCryptoServiceLike)
...
#### createKeyExchangeHandler(manager: TransportEncryptionManager)

创建密钥交换端点处理器，用于客户端与服务端交换 SM2 公钥。

接收 `POST { clientPublicKey: string }` 请求，返回 `{ serverPublicKey, clientId }`。

#### transportEncryptionMiddleware(config: TransportEncryptionConfig)
...
#### isValidEncryptedPayload(payload: unknown)

校验载荷是否为合法的 `EncryptedPayload` 格式（包含 `encryptedKey`、`ciphertext`、`iv` 字符串字段）。

### Storage 模块

#### createStorageEndpoint(config: StorageEndpointConfig)
...
### useSession(options?: UseSessionOptions): SessionStore
...
### useUpload(options?: UseUploadOptions): UploadStore
...
### useIsAuthenticated(sessionStore: SessionStore): Readable\<boolean\>

从 SessionStore 派生的认证状态。

### useUser(sessionStore: SessionStore): Readable\<ClientUser | null\>

从 SessionStore 派生的用户信息。

### useTransportEncryption(options: UseTransportEncryptionOptions): TransportEncryptionStore
...
浏览器请求 → hooks.server.ts handle
  ...
  │
  ├─ createHandle:
  │    ├─ 执行 guards（authGuard / roleGuard / permissionGuard）
  │    ├─ 执行 middleware 链（cors → csrf → logging → rateLimit → ...）
  │    └─ resolve(event) → 生成响应
  │
  ├─ API 端点（+server.ts）:
  │    ├─ 从 event.locals.session 读取会话
  │    ├─ validateForm / validateQuery / validateParams 验证
  │    └─ ok / created / error 返回标准响应
  │
  └─ 响应头: X-Request-Id
```
...
```
useSession → fetch('/api/session') → 获取 user
           → 自动 refreshInterval 刷新
           → logout() → POST /api/logout → 清空状态

useUpload  → addFiles → 队列 → XMLHttpRequest 上传（带进度）
           → 或 presignUrl 模式 → GET 预签名 → PUT 文件
```

## 13. i18n 集成

### getKitMessage(key: KitMessageKey): string

获取 kit 模块的 i18n 消息，自动跟随全局 locale。

### setAllModulesLocale(locale: string): void
...
```typescript
import { badRequest, internalError, ok, unauthorized, validateForm, validationError } from '@hai/kit'
