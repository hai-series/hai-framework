...
| Store      | 统一存储抽象（对话记录、会话、检索源、记忆、上下文持久化）          | RelDB + VecDB（内部子系统）                       |
...
- `@h-ai/reldb` — 数据库（对话记录/会话/记忆/知识库持久化），**需在 ai.init() 前初始化**
- `@h-ai/vecdb` — 向量数据库（记忆向量检索/知识库检索），**需在 ai.init() 前初始化**
...
await vecdb.init({ type: 'sqlite', database: './ai-vec.db' })

// 2. 初始化 AI 模块（自动使用已初始化的 reldb 和 vecdb 单例）
await ai.init({
  llm: {
    model: 'gpt-4o-mini',
