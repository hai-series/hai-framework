...
| Context    | 上下文压缩、摘要、Token 估算、有状态管理器（含会话持久化） | ✅         |
| Store      | 统一存储抽象（内存 / 持久化自动降级）                      | 内部       |
...
ai.init({
  ...
  // 存储配置（Memory / Context 的持久化后端）
  store: {
    mode: 'memory', // 'memory' | 'persistent'
    // persistent 模式需要 @h-ai/reldb + @h-ai/vecdb 已初始化
    // 若 persistent 不可用，自动降级为 memory
  },
  ...
  context: {
    ...
    summaryModel: 'gpt-4o-mini', // 可选，摘要用模型
    apiKey: 'sk-context-xxx', // 可选，覆盖 LLM apiKey
    baseUrl: 'https://context.api.com', // 可选，覆盖 LLM baseUrl
...
```ts
// 编码 SSE 事件
const encoded = ai.stream.encodeSSE({ event: 'message', data: JSON.stringify({ text: 'hello' }) })

// 解码 SSE 数据
const decoder = ai.stream.createSSEDecoder()
for (const event of decoder.decode(rawText)) {
  // event: { event?: string, id?: string, retry?: number, data?: string }
}
```
...
// 查看已注册的源
const sources = ai.retrieval.listSources() // RetrievalSource[]
```
...
```ts
ai.retrieval.removeSource('docs')
```
...
管理对话上下文的 Token 控制：压缩超长对话、生成摘要、Token 估算。支持会话级持久化（配合 Store persistent 模式）。
...
```ts
// 对消息生成摘要
const result = await ai.context.summarize(messages, {
  model: 'gpt-4o-mini',
  temperature: 0.3,
})
...
// 增量摘要（在已有摘要基础上追加）
const updated = await ai.context.summarize(newMessages, {
...
```ts
const result = ai.context.estimateTokens(messages)
if (result.success) {
...
适用于多轮对话，自动跟踪 Token 使用量，超限时自动压缩。

```ts
const managerResult = ai.context.createManager({
  scope: { objectId: 'user-001', sessionId: 'sess-001' }, // 可选，关联对象和会话
  maxTokens: 8000, // Token 预算
  strategy: 'hybrid', // 压缩策略
  preserveSystem: true,
  preserveLastN: 4,
  autoCompress: true, // 超限时自动压缩（默认 true）
})
...
// 追加消息（超限时自动触发压缩）
await manager.addMessage({ role: 'system', content: '你是一个助手' })
...
if (usage.success) {
  // usage.data.current — 当前 Token 数
  // usage.data.budget  — Token 预算
}

// 查看历史摘要
const summaries = manager.getSummaries()
// summaries.data: ContextSummary[]
...
if (result.success) {
  for (const session of result.data) {
    ...
    // session.objectId  — 对象 ID
    // session.createdAt — 创建时间
    // session.updatedAt — 最后更新时间
  }
}
```
...
ai.init({
  ...
  context: { defaultStrategy: 'hybrid', preserveLastN: 6 },
})
...
async function chat(userInput: string): Promise<string | undefined> {
  ...
  // 2. 压缩上下文以适应模型限制
  const compressed = await ai.context.tryCompress(enrichedMessages, {
    maxTokens: 4000,
    strategy: 'hybrid',
  })
