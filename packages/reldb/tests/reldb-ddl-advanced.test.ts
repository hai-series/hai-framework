...
describe('reldb.ddl advanced', () => {
  const defineCommon = (label: 'sqlite' | 'mysql' | 'postgresql') => {
    ...
    it(`${label}: 创建含全部列类型的表应成功`, async () => {
      await reldb.ddl.dropTable('all_types', true)
      const result = await reldb.ddl.createTable('all_types', {
      ...
      const row = await reldb.sql.get('SELECT * FROM all_types WHERE id = 1')
    ...
    it(`${label}: 各种默认值类型应正确生效`, async () => {
      await reldb.ddl.dropTable('defaults_test', true)
      const result = await reldb.ddl.createTable('defaults_test', {
        id: { type: 'INTEGER', primaryKey: true, autoIncrement: true },
      ...
      await reldb.sql.execute('INSERT INTO defaults_test (id) VALUES (?)', [1])

      const row = await reldb.sql.get<{ status: string, priority: number, nullable_col: string | null }>(
    ...
    it(`${label}: 复合索引应生效`, async () => {
      await reldb.ddl.dropTable('events', true)
      await reldb.ddl.createTable('events', {
        id: { type: 'INTEGER', primaryKey: true, autoIncrement: true },
      ...
      const createIdx = await reldb.ddl.createIndex('events', 'idx_events_user_type', {
        columns: ['user_id', 'event_type'],
      })
      expect(createIdx.success).toBe(true)

      // 验证索引可用（通过执行查询）
      await reldb.sql.execute('INSERT INTO events (user_id, event_type) VALUES (?, ?)', [1, 'login'])
      await reldb.sql.execute('INSERT INTO events (user_id, event_type) VALUES (?, ?)', [1, 'logout'])

      const result = await reldb.sql.query<{ event_type: string }>(
    ...
    it(`${label}: 唯一复合索引应阻止重复组合`, async () => {
      await reldb.ddl.dropTable('subscriptions', true)
      await reldb.ddl.createTable('subscriptions', {
        id: { type: 'INTEGER', primaryKey: true, autoIncrement: true },
        user_id: { type: 'INTEGER', notNull: true },
        plan: { type: 'TEXT', notNull: true },
      })

      await reldb.ddl.createIndex('subscriptions', 'idx_sub_user_plan', {
        columns: ['user_id', 'plan'],
        unique: true,
      })

      await reldb.sql.execute('INSERT INTO subscriptions (user_id, plan) VALUES (?, ?)', [1, 'premium'])
      await reldb.sql.execute('INSERT INTO subscriptions (user_id, plan) VALUES (?, ?)', [1, 'basic'])
      await reldb.sql.execute('INSERT INTO subscriptions (user_id, plan) VALUES (?, ?)', [2, 'premium'])

      // 重复的 (user_id, plan) 组合应失败
      const dup = await reldb.sql.execute('INSERT INTO subscriptions (user_id, plan) VALUES (?, ?)', [1, 'premium'])
    ...
    it(`${label}: notNull 列插入 null 应失败`, async () => {
      await reldb.ddl.dropTable('strict_table', true)
      await reldb.ddl.createTable('strict_table', {
        id: { type: 'INTEGER', primaryKey: true, autoIncrement: true },
        required_field: { type: 'TEXT', notNull: true },
      })

      const result = await reldb.sql.execute('INSERT INTO strict_table (required_field) VALUES (?)', [null])
    ...
    it(`${label}: 含外键引用的表应创建成功`, async () => {
      await reldb.ddl.dropTable('comments', true)
      await reldb.ddl.dropTable('posts', true)

      // 不使用 autoIncrement 以避免 MySQL BIGINT/INT 类型不匹配
      await reldb.ddl.createTable('posts', {
        id: { type: 'INTEGER', primaryKey: true },
        title: { type: 'TEXT', notNull: true },
      })

      const result = await reldb.ddl.createTable('comments', {
        id: { type: 'INTEGER', primaryKey: true },
        post_id: {
          type: 'INTEGER',
          notNull: true,
      ...
      // 验证能正常插入
      await reldb.sql.execute('INSERT INTO posts (id, title) VALUES (?, ?)', [1, 'Post 1'])
      const insertComment = await reldb.sql.execute(
    ...
    it(`${label}: addColumn 后应可立即写入和读取新列`, async () => {
      await reldb.ddl.dropTable('dynamic_table', true)
      await reldb.ddl.createTable('dynamic_table', {
        id: { type: 'INTEGER', primaryKey: true, autoIncrement: true },
        name: { type: 'TEXT', notNull: true },
      })

      await reldb.sql.execute('INSERT INTO dynamic_table (name) VALUES (?)', ['旧记录'])

      // 添加新列
      await reldb.ddl.addColumn('dynamic_table', 'score', { type: 'INTEGER', defaultValue: 0 })

      // 旧记录的新列应有默认值
      const oldRow = await reldb.sql.get<{ name: string, score: number }>(
      ...
      // 新记录可以写入新列
      await reldb.sql.execute('INSERT INTO dynamic_table (name, score) VALUES (?, ?)', ['新记录', 100])
      const newRow = await reldb.sql.get<{ name: string, score: number }>(
