...
describe('reldb.sql advanced', () => {
  const defineCommon = (label: 'sqlite' | 'mysql' | 'postgresql') => {
    ...
    it(`${label}: execute INSERT 应返回 lastInsertRowid`, async () => {
      ...
      const r2 = await reldb.sql.execute('INSERT INTO items (title) VALUES (?)', ['Item B'])
    ...
    it(`${label}: UPDATE 多行应返回正确 changes`, async () => {
      ...
      await reldb.sql.execute('INSERT INTO items (title, status) VALUES (?, ?)', ['A', 'draft'])
      await reldb.sql.execute('INSERT INTO items (title, status) VALUES (?, ?)', ['B', 'draft'])
      await reldb.sql.execute('INSERT INTO items (title, status) VALUES (?, ?)', ['C', 'published'])

      const result = await reldb.sql.execute(
    ...
    it(`${label}: DELETE 多行应返回正确 changes`, async () => {
      ...
      await reldb.sql.execute('INSERT INTO items (title) VALUES (?)', ['A'])
      await reldb.sql.execute('INSERT INTO items (title) VALUES (?)', ['B'])
      await reldb.sql.execute('INSERT INTO items (title) VALUES (?)', ['C'])

      const result = await reldb.sql.execute('DELETE FROM items WHERE id <= ?', [2])
      expect(result.success).toBe(true)
      if (result.success) {
        expect(result.data.changes).toBe(2)
      }
      ...
      // 验证只剩 1 条
      const remaining = await reldb.sql.query('SELECT * FROM items')
    ...
    it(`${label}: 违反 NOT NULL 约束应返回失败`, async () => {
      ...
      const result = await reldb.sql.execute('INSERT INTO items (title) VALUES (?)', [null])
    ...
    it(`${label}: 违反 UNIQUE 约束应返回失败`, async () => {
      ...
      await reldb.sql.execute('INSERT INTO items (code) VALUES (?)', ['CODE-001'])
      const dup = await reldb.sql.execute('INSERT INTO items (code) VALUES (?)', ['CODE-001'])
    ...
    it(`${label}: batch 混合 INSERT/UPDATE/DELETE 应全部生效`, async () => {
      ...
      // 先插入两条基础数据
      await reldb.sql.execute('INSERT INTO items (title, status) VALUES (?, ?)', ['Alpha', 'draft'])
      await reldb.sql.execute('INSERT INTO items (title, status) VALUES (?, ?)', ['Beta', 'draft'])

      // batch: 插入 + 更新 + 删除
      const result = await reldb.sql.batch([
      ...
      const rows = await reldb.sql.query<{ title: string, status: string }>(
    ...
    it(`${label}: batch 包含无效 SQL 应整体失败`, async () => {
      ...
      await reldb.sql.execute('INSERT INTO items (title) VALUES (?)', ['Original'])
      ...
      const items = await reldb.sql.query<{ title: string }>('SELECT title FROM items')
    ...
    it(`${label}: REAL 类型应正确存取浮点数`, async () => {
      await reldb.ddl.dropTable('metrics', true)
      await reldb.ddl.createTable('metrics', {
        id: { type: 'INTEGER', primaryKey: true, autoIncrement: true },
        value: { type: 'REAL', notNull: true },
      })

      await reldb.sql.execute('INSERT INTO metrics (value) VALUES (?)', [3.14159])

      const result = await reldb.sql.get<{ value: number }>('SELECT value FROM metrics WHERE id = 1')
      expect(result.success).toBe(true)
      if (result.success) {
    ...
    it(`${label}: 列默认值应在未提供值时生效`, async () => {
      await reldb.ddl.dropTable('configs', true)
      await reldb.ddl.createTable('configs', {
        id: { type: 'INTEGER', primaryKey: true, autoIncrement: true },
      ...
      await reldb.sql.execute('INSERT INTO configs (config_key) VALUES (?)', ['test_key'])

      const result = await reldb.sql.get<{ config_key: string, value: string, priority: number }>(
    ...
    it(`${label}: queryPage overrides.maxPageSize 应截断超大 pageSize`, async () => {
      ...
      for (let i = 1; i <= 5; i++) {
        await reldb.sql.execute('INSERT INTO items (title) VALUES (?)', [`Item ${i}`])
      }

      const result = await reldb.sql.queryPage<{ id: number }>({
    ...
    it(`${label}: 查询结果应正确返回多列不同类型`, async () => {
      await reldb.ddl.dropTable('products', true)
      ...
      const result = await reldb.sql.get<{ id: number, name: string, price: number, active: number | boolean }>(
