...
describe('reldb.sql', () => {
  const defineCommon = (label: 'sqlite' | 'mysql' | 'postgresql') => {
    it(`${label}: execute/query/get 应该按预期返回结果`, async () => {
      ...
      const insert = await reldb.sql.execute('INSERT INTO users (name) VALUES (?)', ['用户1'])
      expect(insert.success).toBe(true)
      if (insert.success) {
        expect(insert.data.changes).toBe(1)
      }

      const users = await reldb.sql.query<{ id: number, name: string }>('SELECT id, name FROM users')
      ...
      const user = await reldb.sql.get<{ id: number, name: string }>('SELECT id, name FROM users WHERE id = ?', [1])
    ...
    it(`${label}: query 无匹配行应返回空数组`, async () => {
      ...
      const result = await reldb.sql.query<{ id: number }>('SELECT id FROM users')
    ...
    it(`${label}: get 无匹配行应返回 null`, async () => {
      ...
      const result = await reldb.sql.get<{ id: number }>('SELECT id FROM users WHERE id = ?', [999])
      expect(result.success).toBe(true)
      if (result.success) {
    ...
    it(`${label}: execute 无匹配行应返回 changes=0`, async () => {
      ...
      const result = await reldb.sql.execute('DELETE FROM users WHERE id = ?', [999])
      expect(result.success).toBe(true)
      if (result.success) {
    ...
    it(`${label}: execute 无效 SQL 应返回失败`, async () => {
      const result = await reldb.sql.execute('INSERT INTO nonexistent_table_xyz (name) VALUES (?)', ['用户'])
      expect(result.success).toBe(false)
    })
    ...
    it(`${label}: 参数化查询应正确绑定`, async () => {
      ...
      await reldb.sql.execute('INSERT INTO users (name) VALUES (?)', ['Alice'])
      await reldb.sql.execute('INSERT INTO users (name) VALUES (?)', ['Bob'])
      await reldb.sql.execute('INSERT INTO users (name) VALUES (?)', ['Charlie'])

      const result = await reldb.sql.query<{ name: string }>(
