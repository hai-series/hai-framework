...
describe('reldb.sql.queryPage', () => {
  const defineCommon = (label: 'sqlite' | 'mysql' | 'postgresql') => {
    it(`${label}: 应返回正确分页`, async () => {
      ...
      await reldb.sql.execute('INSERT INTO users (name) VALUES (?)', ['用户1'])
      await reldb.sql.execute('INSERT INTO users (name) VALUES (?)', ['用户2'])
      await reldb.sql.execute('INSERT INTO users (name) VALUES (?)', ['用户3'])

      const firstPage = await reldb.sql.queryPage<{ id: number, name: string }>({
      ...
      const secondPage = await reldb.sql.queryPage<{ id: number, name: string }>({
        sql: 'SELECT id, name FROM users ORDER BY id ASC',
    ...
    it(`${label}: 带 WHERE 参数的分页应正确`, async () => {
      ...
      await reldb.sql.execute('INSERT INTO users (name, status) VALUES (?, ?)', ['用户1', 'active'])
      await reldb.sql.execute('INSERT INTO users (name, status) VALUES (?, ?)', ['用户2', 'inactive'])
      await reldb.sql.execute('INSERT INTO users (name, status) VALUES (?, ?)', ['用户3', 'active'])
      await reldb.sql.execute('INSERT INTO users (name, status) VALUES (?, ?)', ['用户4', 'active'])

      const result = await reldb.sql.queryPage<{ id: number, name: string }>({
    ...
    it(`${label}: overrides 应修改默认分页参数`, async () => {
      ...
      for (let i = 1; i <= 10; i++) {
        await reldb.sql.execute('INSERT INTO users (name) VALUES (?)', [`用户${i}`])
      }
