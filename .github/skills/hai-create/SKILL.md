...
export const xx: XxFunctions = {
  async init(config: XxConfigInput): Promise<Result<void, XxError>> {
    ...
    try {
      ...
      if (!yyResult.success) { return yyResult }
...
export function createTypeAProvider(): XxProvider {
  ...
  const zz: ZzOperations = {
    async create(data) {
      try { return ok(await client!.insert(data)) }
      catch (error) { return err(toXxError(error)) }
    },
    async get(id) {
      try { return ok(await client!.get(id)) }
      catch (error) { return err(toXxError(error)) }
    },
    async remove(id) { /* ... */ },
  }
...
```ts
// xx-types.ts 底部追加
export type { XxYyFunctions, YyItem, CreateYyInput } from './yy/xx-yy-types.js'
...
```ts
// ❌ return 中嵌套复杂逻辑
return db.query(...).then(r => r.success ? ok(r.data.map(...)) : err(...))
...
```ts
// ✅ 接口聚合
interface ServiceDeps { sql: SqlOperations; config: XxConfig }
...
1. **模块概述** — 一句话描述
2. **入口与初始化** — import / init / close / isInitialized
3. **目录结构** — 完整文件树
4. **配置说明** — 所有字段、类型、默认值
5. **操作接口** — 完整方法签名、参数、返回值
6. **Client 接口**（如有） — 创建方式、方法列表
7. **错误码** — 数值 + 含义
8. **注意事项** — 关键限制和边界
