...
> **优先级固定**：先满足真实需求，再考虑扩展；先简单，再抽象。若“更通用”和“更清晰”冲突，默认选择更清晰。

### 简单优先（Less is More）

- 先用最少目录、最少接口、最少配置、最少层级满足当前需求。
- 能放在现有模块/现有文件解决，就不要新增一层概念。
- 一个清晰实现优先于“理论上更灵活”的多层封装。

### 最小知识原则（Law of Demeter）

- 使用方只需要知道公共入口 `xx`、必要配置和业务返回值，不需要理解内部目录、Provider 细节和装配过程。
- 对外类型只暴露业务字段，不暴露 DB 行结构、内部状态和中间对象。
- 若多个子特性依赖同一上游，优先顶层注入一次，由模块内部派生所需能力，避免使用方重复转发。

### 整洁代码（Clean Code）

- 命名必须直白表达职责，函数主流程必须一眼可读。
- 单一职责、early return、少嵌套、少样板，抽象必须实质降低复杂度。
- 死代码、注释掉代码、一次性包装层、纯转发样板都应删除或折叠。

### 反过度设计（YAGNI）

- 没有第二个真实实现，不引入 Provider / 策略模式 / 抽象基类。
- 没有真实使用场景，不新增配置项、扩展点、兼容层、开关。
- 没有跨文件复用，不提前拆出 utils / constants / helper 文件。
- 没有对外使用场景，不新增导出。

### 创建前四问（必须先答）

1. 如果不增加这个抽象 / 子目录 / 配置项，模块是否会更简单？
2. 仓库里是否已有可复用的模式、模块或实现，而不是再造一个“更通用版本”？
3. 使用方是否会因为这次设计被迫理解内部结构？
4. 这个扩展点是否有明确、当前、可验证的真实使用场景？
...
| **基础设施模块** | 提供底层能力（日志、配置、i18n、HaiResult 等）   | core                                                                                                           |
...
- **没有两个明确操作域** → 不拆子操作对象，优先扁平 API
- **没有跨文件复用** → 不提前提取 utils / helper / constants 文件
- **没有真实 consumer** → 不导出类型、函数、配置项
- **只是转发上游能力** → 不要求使用方手写样板转发，模块内部自行派生
...
>
> **强约束**：Provider 是高成本抽象。只有存在两个及以上真实后端，或已确认近期落地第二后端时才使用；禁止“先抽出来以后可能会用”。
...
> **默认优先扁平 API**：只有分组能明显降低理解成本时，才使用子操作对象。少暴露一个层级，通常优于多一个“看起来整齐”的 getter。
...
export interface XxFunctions {
  ...
  close: () => Promise<void>
  doSomething: (input: Input) => Promise<HaiResult<Output>>
  doAnother: (id: string) => Promise<HaiResult<void>>
...
- 示例应展示最常见的调用方式，包含必要的错误处理（HaiResult 判断）
...
- 先删除冗余，再复用现有能力，再收敛概念，最后才新增抽象。
- 抽象必须降低复杂度；如果只是把复杂度从实现端转移到调用方，说明抽象失败。
- 任何“为了以后可能支持”而新增的层级、接口、配置，都默认视为坏味道。
...
公共 API 不 throw，返回 `HaiResult<T>`。允许 throw 的合规场景：
...
| async generator（如 `chatStream()`） | 无法返回 HaiResult，需在 JSDoc 中注明    |
...
- ❌ 为未来假设添加未使用的接口 / 扩展点 / 配置项 / 兼容层
- ❌ 只有单一实现时引入 Provider / Strategy / Base Class
- ❌ 只为转发上游方法而要求使用方写样板代码
- ❌ return 嵌套复杂逻辑
- ❌ 超过 2 层 if 嵌套
- ❌ 重新包装上游 HaiResult 错误
...
- [ ] 已完成“最小知识 / Less is More / 整洁代码 / YAGNI”自检
- [ ] 无为未来假设新增的抽象 / 配置 / 兼容层
- [ ] 使用方只需要认识公共入口，不需要理解内部结构
