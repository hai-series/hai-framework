...
- 使用或扩展 `db.crud.table` / `BaseCrudRepository`。
- 处理分页查询或分页结果规范化。
- 需要基于 `DbErrorCode` 做错误分支处理。
...
4. **SQL 规则与参数**
   - 参数占位符统一使用 `?`。
   - 分页查询使用 `queryPage`（或 `db.pagination` 构建结果）。

5. **错误处理**
   - 使用 `DbErrorCode` 做分支判断（如 `NOT_INITIALIZED`）。
...
- `db.init` 未调用时，`db.ddl/db.sql/db.tx` 返回 `NOT_INITIALIZED`。
- CRUD 的 `create/update` 在 payload 为空时会返回 `CONFIG_ERROR`。
- SQLite 使用 `database` 作为文件路径或 `:memory:`。
- `db.pagination.normalize` 会裁剪 `pageSize`，避免过大分页。
